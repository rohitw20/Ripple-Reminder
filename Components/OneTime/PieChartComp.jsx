  ...
  React.useEffect(() => {
    ...
    const calculatePercentage = async () => {
      ...
      const allTasks = await db.getAllAsync(
        `SELECT taskid FROM ONETIME WHERE EXPIRY=DATE('NOW') AND ISDELETED=0`
      );

      const completedTasks = await db.getAllAsync(
        `SELECT taskid FROM ONETIME WHERE EXPIRY=DATE('NOW') AND ISDELETED=0 AND STATUS='complete'`
      );
