...
const calculatePercentage = async (date) => {
  ...
  const allTasks = await db.getFirstAsync(
    `SELECT count(*) as count FROM ONETIME WHERE EXPIRY=? AND ISDELETED=0`,
    date
  );

  const completedTasks = await db.getFirstAsync(
    `SELECT count(*) as count FROM ONETIME WHERE EXPIRY=? AND STATUS='complete'`,
    date
  );
  ...
  const result =
    (completedTasks.count * 100) / (allTasks.count !== 0 ? allTasks.count : 1);
